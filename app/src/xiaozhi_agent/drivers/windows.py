...
def open_app(name: str) -> str:
    target = _exe_for(name)
    try:
        subprocess.Popen([target], shell=False)
    except FileNotFoundError:
        if os.name != "nt":
            raise
        # ShellExecute/App Paths can resolve installed GUI apps such as Office
        # without routing user text through cmd.exe.
        os.startfile(target)  # type: ignore[attr-defined]
    return f"已尝试打开 {name}"


def close_app(name: str) -> str:
    target = Path(_exe_for(name)).name.lower()
    target_stem = target.removesuffix(".exe")
    killed = 0
    for p in psutil.process_iter(["name"]):
        try:
            pname = (p.info.get("name") or "").lower()
            pname_stem = pname.removesuffix(".exe")
            if pname == target or pname_stem == target_stem:
                p.terminate()
                killed += 1
